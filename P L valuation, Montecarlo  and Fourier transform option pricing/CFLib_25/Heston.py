...
def Gamma( c_L, c_kappa, sigma):
    return np.sqrt(c_kappa*c_kappa + 2.*c_L*pow(sigma,2))
#
...
# ----------------------------------------
    ...
    def log_cf(self, c_k, t): 
        if np.all(c_k.real == 0.0): 
            return np.zeros_like(c_k, dtype=complex)
    ...
    def HestonPut( self
        ...
        Fw = So*exp((r-q)*T)
        kT = Strike / Fw
        return exp(-r*T)*Fw*ft_opt(self, kT, T, Xc)["put"]
    #----------------------------------------------
    def var_log_martingale(self, T):
        v0 = self.nu_o
        kappa = self.lmbda
        theta = self.nubar
        sigma = self.eta
        rho = self.rho
        term1 = v0 * (1 - np.exp(-kappa * T)) / (kappa * T)
        term2 = theta * (T - (1 - np.exp(-kappa * T)) / kappa)
        return term1 + term2  # approx. of Var[log M_T]
