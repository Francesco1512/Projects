...
def bachelier_option_price(forward, strike, vol, time_to_expiry, w):
    """
    Bachelier (normal) formula for a single caplet/floorlet (European call or put on the forward).
    ...
    --------
    float
        Price of the caplet/floorlet under the Bachelier model.
    """
    # If vol is extremely small, approximate by intrinsic value.
    if vol < 1e-14:
        intrinsic = max(w * (forward - strike), 0)
        return intrinsic

    # Bachelier formula
    d = (forward - strike) / (vol * sqrt(time_to_expiry))
    # w=+1 => caplet (call); w=-1 => floorlet (put)
    price = w * ((forward - strike) * norm.cdf(w * d)) + vol * sqrt(time_to_expiry) * norm.pdf(d)
    return price
...
def price_cap_floor(discount_curve, forward_curve, maturity_Tn, strike, w, vol):
    ...
    for i in range(2, i_max + 1):
        ...
        caplet_price = bachelier_option_price(F_i, strike, vol, T_i, w)
...
def implied_vol_cap_floor(mkt_price, discount_curve, forward_curve, maturity_Tn, strike, w):
    """
    Numerically invert the Bachelier price to find the implied normal volatility.
...
######################################################################
# 4) Main script: read data, compute Normal implied vols, pivot
######################################################################
def main():
    ...
    for idx, row in inputs_df.iterrows():
        ...
        results.append({
            ...
            "implied_n_vol": vol  # now it's a normal vol under the Bachelier model
        })

    results_df = pd.DataFrame(results)
    print("\n--- Full Results (Bachelier, one row per maturity-strike-omega) ---")
    print(results_df)
    ...
    matrix_df = results_df.pivot_table(
        index="maturity",
        columns="strike",
        values="implied_n_vol"
    )

    print("\n--- Normal Implied Vol Surface (matrix) ---")
    print(matrix_df)

    # Save to Excel
    output_file = "./advanced interest rate project 2/normal_implied_vols_output.xlsx"
